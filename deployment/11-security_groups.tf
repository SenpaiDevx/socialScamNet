resource "aws_security_group" "bastion_host_sg" {
  ...
  ingress {
    ...
    to_port     = 22
    protocol    = "TCP"
    cidr_blocks = [var.bastion_host_cidr] # "var.bastion_host_cidr" give owner pc ip 192.168.1.200 which is not valid cidr block instead tp fix "192.168.1.200/32" for specific ip
    description = "Allows SSH into bastion host instance"
  }
