...
class HealthRoutes {
    ...
    health() {
        this.router.get('/health', (req, res) => {
            res.status(http_status_codes_1.default.OK).send(`Health: Server instance is healthy with process id ${process.pid} on ${(0, moment_1.default)().format('LL')}`);
        });
        return this.router;
    }
    env() {
        this.router.get('/env', (req, res) => {
            res.status(http_status_codes_1.default.OK).send(`This is the ${config_1.config.NODE_ENV} environment.rfrgrg5t3e2e3e3r4g5g`);
        });
        return this.router;
    }
    instance() {
    ...
    fiboRoutes() {
        this.router.get('/fibo/:num', (req, res) => __awaiter(this, void 0, void 0, function* () {
            ...
            res
                .status(http_status_codes_1.default.OK)
                .send(`Fibonacci series of ${num} is ${result} and it took ${end - start}ms and runs with process id ${process.pid} on ${0} at ${(0, moment_1.default)().format('LL')}`);
        }));
        return this.router;
    }
    fibo(data) {
