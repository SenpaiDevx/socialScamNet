...
class FollowerService {
  public async addFollowerToDB(userId: string, followeeId: string, username: string, followerDocumentId: ObjectId): Promise<void> {
    ...
    const users: Promise<BulkWriteResult> = UserModel.bulkWrite([ // multiple field update one command
      {
        updateOne: {
          filter: { _id: userId },
  ...
  public async getFolloweeData(userObjectId: ObjectId): Promise<IFollowerData[]> {
    const followee: IFollowerData[] = await FollowerModel.aggregate([
      ...
      { $lookup: { from: 'User', localField: 'followeeId', foreignField: '_id', as: 'followeeId' } }, // join tabbe Follower TB = User TB
      { $unwind: '$followeeId' }, // get specific item 
      { $lookup: { from: 'Auth', localField: 'followeeId.authId', foreignField: '_id', as: 'authId' } },
      { $unwind: '$authId' },
      {
        $addFields: { // creating new field as result 
          _id: '$followeeId._id', // from joining 2 tables using $unwind operator we can create value act as access modifier of each field of joining table
      ...
      {
        $project: { // remove fields that are not needed or transform the result 
          authId: 0,
          followerId: 0,
          followeeId: 0,
          createdAt: 0,
