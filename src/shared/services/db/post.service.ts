...
class PostService {
    ...
    public async postsCount(): Promise<number> { // returns total posts count
        const count: number = await PostModel.find({}).countDocuments(); // find() with empty query returns all documents
        return count;
    };
