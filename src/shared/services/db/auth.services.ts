...
class AuthService {
    ...
    public async getAuthUserByUsername(username: string): Promise<IAuthDocument> { // findOne by get only the username
        const user: IAuthDocument = await AuthModel.findOne({ username: Helpers.firstLetterUppercase(username) }).exec() as IAuthDocument; // you to refer this value as type of IAuthDocument using "as" keyword
        return user;
    }
