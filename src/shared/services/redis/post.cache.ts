...
export class PostCache extends BaseCache {
  ...
  public async savePostToCache(data: ISavePostToCache): Promise<void> {
    ...
    const { _id, userId, username, email,
      avatarColor, profilePicture, post, bgColor,
      commentsCount, imgVersion, imgId, videoId,
      videoVersion, feelings, gifUrl, privacy,
      reactions, createdAt
    } = createdPost;
    ...
    try {
      ...
      for (const [itemKey, itemValue] of Object.entries(dataToSave)) {
        await multi.HSET(`posts:${key}`, `${itemKey}`, `${itemValue}`);
      }
      var count: number = parseInt(postCount[0], 10) + 1;
  ...
  public async getPostsFromCache(key: string, start: number, end: number): Promise<IPostDocument[]> {
    try {
      ...
      const reply: string[] = await this.client.zRange(key, start, end,) // retrieve example : 0 -> 2 hash set data. if 0 -> -1 all set will retrieve
      console.log(reply)
      const multi: ReturnType<typeof this.client.multi> = this.client.multi();
      ...
      for (const post of replies as IPostDocument[]) {
        ...
        post.reactions = Helpers.parseJson(`${post.reactions}`) as IReactions; // casting to IReactions will temporary later to create interface of folder reactions
      ...
      log.error(error);
      throw new ServerError('Server error. Try again. plsss :' + error);
    }
  }

  public async getTotalPostsInCache(): Promise<number | undefined> {
    try {
      if (!this.client.isOpen) {
        await this.client.connect();
  ...
  public async getPostsWithImagesFromCache(key: string, start: number, end: number): Promise<IPostDocument[] | any> {
    try {
      if (!this.client.isOpen) {
        await this.client.connect();
